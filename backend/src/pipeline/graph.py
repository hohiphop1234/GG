...
from src.nodes.rewriter import QueryRewriter
from src.nodes.router import QueryClassification
from src.nodes.guard import get_disclaimer, SafetyGuard
from src.store.embeddings import EmbeddingManager
from src.store.vector_store import VectorStore
from src.store.bm25_store import BM25Store
from src.store.retriever import HybridRetriever
from src.nodes.grader import EvidenceGrader
from src.store.crawler import WebCrawler
from src.nodes.generator import ResponseGenerator
from src.nodes.validator import ResponseValidator
from src.nodes.router import QueryRouter

FAQ_SYSTEM_PROMPT = _load_prompt("faq.txt")
...
class LangGraphPipeline:
    ...
    def intent_router_node(self, state: MedicalState) -> dict:
        ...
        # 1. Khẩn cấp (Emergency): Regex OR PhoBERT
        is_regex_emergency = self.safety_guard.is_emergency(question)
        if is_regex_emergency or classification.category == "emergency":
            if is_regex_emergency and classification.category != "emergency":
                logger.info(f"[{question}] SafetyGuard detected emergency via Regex, but PhoBERT classified as {classification.category}. Upgrading to emergency.")
            elif not is_regex_emergency and classification.category == "emergency":
                logger.info(f"[{question}] PhoBERT classified as emergency, SafetyGuard Regex did not match. Flagging as emergency for safety.")
            
            emergency_resp = self.safety_guard.emergency_response(question)
            return {
                "category": "emergency",
                "risk_level": "critical",
                "confidence": 1.0 if is_regex_emergency else classification.confidence,
                "route": "early_exit",
                "exit_type": "emergency",
    ...
    def web_retrieval_node(self, state: MedicalState) -> dict:
        ...
        prompt = _load_prompt("web_search_agent.txt")
        prompt += f"\n\nOriginal User Question: '{question}'\nRewritten Query: '{rewritten_query}'\nPrevious Failed Attempts: {crawl_iterations}"
